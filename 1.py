# ===================================================================================
#      AI 招聘助理 v20.0 - Fedora 高并发版 (Ollama 最终修复版 v1.0)
# ===================================================================================
# 版本亮点:
# - [AI引擎本地化] 核心分析模型从 Google Gemini API 替换为本地运行的 Ollama (Llama 3)。
# - [Embedding本地化] Embedding 模型从 SentenceTransformer 替换为本地运行的 Ollama (EmbeddingGemma)。
# - [全功能修复] 本版本整合了之前所有问题的修复，包括模块导入、JSON解析、变量定义等。
# ===================================================================================
...
import shutil  # <-- 修复 shuttle 未定义问题
import hashlib
...
async def load_active_jd_from_notion_async():
    ...
    try:
        ...
        if not jds:
            print(f"⚠️ [JD加载器] 所有 'Active' 的JD页面内容均为空。")
            return False
...
def clean_json_response(text):
    ...
    if start_index == -1 or end_index == -1 or end_index < start_index:
        print("   !! [JSON清洁工] 未在模型输出中找到有效的 JSON 结构。")
        return "{}"
...
def read_file_content(file_path): # <-- 恢复被删除的函数
    try:
...
if __name__ == "__main__":
    # 确保 shutil 模块在主执行块中被导入
    import shutil

    async def main():
        os.system('cls' if os.name == 'nt' else 'clear')
        print("="*10 + " AI 招聘助理 v20.0 - Fedora 高并发版 (本地化) " + "="*10)
        print(">> 系统启动: 正在初始化本地模型和数据库连接...")
        print("-" * 70)
        ...
        if ENABLE_GOOGLE_SEARCH and (not GOOGLE_API_KEY or "AIza" not in GOOGLE_API_KEY or not GOOGLE_CSE_ID):
            print("⚠️ [警告] Google搜索已启用，但 .env 文件中的API Key或CSE ID未正确配置。")
            ENABLE_GOOGLE_SEARCH = False
