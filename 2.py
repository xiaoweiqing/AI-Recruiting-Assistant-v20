...
def parse_rocket_output(text):
    """一个专门解析 ROCKET_PROMPT_V1 输出的简单解析器"""
    data = {}
    ...
    for line in lines:
        if ":" in line:
            ...
            elif key == "email": data["email"] = value
            elif key == "phone": data["phone"] = value
            elif key == "match_score": data["score"] = int(re.search(r'\d+', value).group() or 0)
            elif key == "reason": data["reason"] = value
    return data
...
def configure_llm():
    global LLM
    try:
        print(">> [AI大脑] 初始化本地 LLM (Ollama/llama3:8b)...")
        LLM = ChatOllama(model="llama3:8b", temperature=0.0) # Temperature 设为 0，追求速度和一致性
        LLM.invoke("Hi") 
        print("✅ [AI大脑] 已准备就绪。")
        return True
    except Exception as e:
...
async def process_single_resume_async(file_path, all_jds_data, worker_name):
    ...
    for jd_title, jd_data in all_jds_data.items():
        prompt = ROCKET_PROMPT_V1.format(jd_text=jd_data['content'], resume_text=resume_text)
        
        response = await LLM.ainvoke(prompt)
        parsed_data = parse_rocket_output(response.content)

        if parsed_data.get("score", 0) > best_match["score"]:
    ...
    print(f">> [{worker_name}] 匹配完成! 最佳职位: '{best_match['best_fit']}' (得分: {best_match['score']})")
        ...
        try:
            ...
            try:
                ...
                if isinstance(result, str) and result.startswith("skipped"):
                    ...
                    print(f"✅ [{name}] 候选人 '{report['name']}' ({filename}) 分析同步完成。")
...
async def batch_mode_high_concurrency():
    ...
    if processed_summaries:
        ...
        for summary in sorted(processed_summaries, key=lambda x: x['score'], reverse=True):
            name = (summary['name'][:17] + '...') if len(summary['name']) > 20 else summary['name']
            pos = (summary['position'][:27] + '...') if len(summary['position']) > 30 else summary['position']
