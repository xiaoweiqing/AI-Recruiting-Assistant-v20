...
async def setup_qdrant_and_embedding():
    ...
    try:
        ...
        # LangChain 的 embed_query 是同步的，如果要在异步函数里调用，最好用 to_thread
...
# ==============================================================================
# ⬇⬇⬇ Notion 交互 & 向量化 (后续所有函数都已经是 async, 无需修改) ⬇⬇⬇
# ==============================================================================
async def check_candidate_existence_async(async_notion, candidate_name, candidate_email):
    if not CANDIDATE_PROFILE_HUB_DB_ID:
        return None
    
    # 【【【 核心修复：确保 db_filter 总能被定义 】】】
    db_filter = {"or": []}

    # 使用 safe get 来避免 NoneType 错误
    safe_email = str(candidate_email or '').lower()
    safe_name = str(candidate_name or '')

    if safe_email and safe_email != 'n/a':
        db_filter["or"].append({
            "property": "候选人邮箱",  # 请确保这个属性名在你的Notion数据库里存在
            "email": {"equals": candidate_email}
        })
    
    if safe_name and safe_name not in ['N/A', '未知候选人', '']:
        db_filter["or"].append({
            "property": "姓名", # 请确保这个属性名在你的Notion数据库里存在
            "title": {"equals": candidate_name}
        })
    ...
    try:
        response = await async_notion.databases.query(
            database_id=CANDIDATE_PROFILE_HUB_DB_ID,
            filter=db_filter,
            page_size=1
        )
        if response and response['results']:
            ...
            return {
                ...
                "analysis_page_id": analysis_page_id,
                "old_hash": old_hash
            }
    except Exception as e:
        # 打印更详细的错误，帮助调试
...
# ... 后续的主执行流程, worker, batch_mode, main 等都保持不变 ...
...
async def main():
    ...
    print("="*10 + " AI 简历信息提取助理 v2.2 (Async 修正) " + "="*10)
    print(">> 系统启动中...")
    print("-" * 70)
    if not await configure_llm(): sys.exit(1)
    await setup_qdrant_and_embedding() # 现在这里是正确的
