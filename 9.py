# ===================================================================================
#      AI 简历信息提取助理 v2.5 (深度提取与归档版)
# ===================================================================================
# v2.5 更新:
# - [通知优化] 采纳 Fedora/Linux 原生 `notify-send` 命令，移除 `plyer` 依赖。
# - [提取升级] 全面重构 AI Prompt，要求模型提取更详细的结构化信息 (JSON格式)。
# - [Notion归档] 实现强大的双数据库同步逻辑：
#   1. 在“分析报告”页面正文中存入 AI 提取的【完整 JSON 数据】。
#   2. 智能填充“候选人信息库”中的详细属性，有则填，无则空。
# v2.4 更新:
...
CANDIDATE_DB_ID = os.getenv("CANDIDATE_DB_ID") # 分析报告数据库 ID
CANDIDATE_PROFILE_HUB_DB_ID = os.getenv("CANDIDATE_PROFILE_HUB_DB_ID") # 候选人信息库 ID
TRAINING_HUB_DATABASE_ID = os.getenv("TRAINING_HUB_DATABASE_ID")

# <<< 关键修改：请确保这里的属性名和您 Notion 数据库中的列名完全一致！ >>>
NOTION_PROPS = {
    # 分析报告库中的属性
    "analysis_name": "候选人姓名", 
    "analysis_date": "分析日期", 
    ...
    # 候选人信息库中的属性 (新增了更多字段)
    "profile_name": "姓名", 
    "profile_relation_to_analysis": "关联分析报告", # 用于关联到分析报告库
    "profile_core_skills": "核心能力或技能", 
    "profile_experience_years": "岗位经验", # 建议此字段类型为 "数字"
...
# ==============================================================================
# ⬇⬇⬇ AI 信息提取 (Prompt & 解析 - 已全面升级) ⬇⬇⬇
# ==============================================================================
# 新的、要求更详细信息和 JSON 输出的 Prompt
EXTRACT_PROMPT_V2 = """
TASK: You are an expert HR assistant. Your task is to extract detailed, structured information from the provided resume text.

RULES:
1. Your entire output MUST be a single, valid JSON object. Do not include any text before or after the JSON.
2. Extract as much information as possible. If a piece of information is not found in the resume, use `null` or an empty string `""` for that field. Do not invent information.
3. For `work_experience_years`, provide a numerical value representing the total years of relevant work experience.
4. `core_skills` should be a comma-separated list of key technologies and abilities.
5. `full_summary` should be a concise paragraph (3-4 sentences) summarizing the candidate's professional background, key achievements, and career focus.
...
{{
  ...
  "current_location": "...",
  "expected_salary": "...",
  "core_skills": "Python, Docker, Agile, Team Leadership, ...",
  "full_summary": "A brief professional summary paragraph..."
}}
...
def show_desktop_notification(title, message):
    """
    使用 subprocess.Popen 以非阻塞方式发送桌面通知，专为 Fedora/Linux 优化。
    """
    print(f"\n🔔 [提醒] 准备发送通知: '{title}'")
    if platform.system() == "Linux":
        try:
            # 使用 Popen “即发即忘”，不阻塞主程序
            subprocess.Popen(
                ['notify-send', title, message, '-a', 'AI简历助理', '-t', '8000']
            )
            print("✅ [提醒] 通知命令已成功发出。")
...
async def setup_qdrant_and_embedding():
    ...
    try:
        ...
        if RECRUITMENT_COLLECTION_NAME not in [c.name for c in collections_response.collections]:
            ...
            await asyncio.to_thread(QDRANT_CLIENT.recreate_collection, collection_name=RECRUITMENT_COLLECTION_NAME, vectors_config=models.VectorParams(size=len(vector_size), distance=models.Distance.COSINE))
...
async def configure_llm():
    ...
    try:
        ...
        print("   -> 正在测试与 Ollama 服务的连接..."); await LLM.ainvoke("Hi")
        print("✅ [AI大脑] 已准备就绪。"); return True
    except Exception as e:
        print(f"❌ [AI大脑] 初始化失败: {e}"); print(f"   -> 请确保 Ollama 服务正在后台运行，并且模型 '{OLLAMA_LLM_MODEL_NAME}' 已经通过 'ollama pull' 成功下载。"); return False
...
def format_json_for_notion(data: dict) -> list:
    """将JSON数据格式化为Notion代码块，以便写入页面正文。"""
    if not data:
        return []
    
    pretty_json = json.dumps(data, indent=2, ensure_ascii=False)
    # Notion API 对单个块有2000字符的限制，需要切分
...
async def save_to_notion_async(async_notion, report_data: dict, resume_text: str, filename: str):
    """
    全新的、强大的Notion同步函数。
    1. 在分析库创建一个页面，并将完整提取的JSON写入页面正文。
    2. 在信息库创建一个详细档案，并智能填充所有可用属性。
    3. 将两个页面关联起来。
    """
    if not report_data:
        print("   !! [Notion同步] 报告数据为空，跳过同步。")
        return None, None
        
    name = report_data.get('name', '未知候选人')
    beijing_time = datetime.now(timezone.utc).astimezone(timezone(timedelta(hours=8)))
    analysis_page_id = None
    profile_page_id = None

    # --- 步骤 1: 创建分析报告页，并写入完整JSON到正文 ---
    try:
        analysis_props = {
        ...
        # 创建页面，但不包含正文内容
        new_analysis_page = await async_notion.pages.create(
            parent={"database_id": CANDIDATE_DB_ID},
            properties={k: v for k, v in analysis_props.items() if v}
        )
        ...
        print(f"   !! [Notion] 操作分析报告库失败: {e}")
        traceback.print_exc()

    # --- 步骤 2: 创建详细的候选人档案页，并智能填充属性 ---
    if analysis_page_id:
        try:
            profile_props = {
            ...
            # --- 智能填充 ---
            if report_data.get("core_skills"):
                profile_props[NOTION_PROPS["profile_core_skills"]] = {"rich_text": [{"text": {"content": report_data["core_skills"]}}]}
            if report_data.get("education"):
                profile_props[NOTION_PROPS["profile_education"]] = {"rich_text": [{"text": {"content": report_data["education"]}}]}
            if report_data.get("current_location"):
                profile_props[NOTION_PROPS["profile_current_location"]] = {"rich_text": [{"text": {"content": report_data["current_location"]}}]}
            if report_data.get("expected_salary"):
                profile_props[NOTION_PROPS["profile_expected_salary"]] = {"rich_text": [{"text": {"content": report_data["expected_salary"]}}]}
            if report_data.get("work_experience_years") is not None:
                try:
                    years = float(report_data["work_experience_years"])
                    profile_props[NOTION_PROPS["profile_experience_years"]] = {"number": years}
                except (ValueError, TypeError):
                    pass # 如果无法转换为数字，则忽略

            new_profile_page = await async_notion.pages.create(
                parent={"database_id": CANDIDATE_PROFILE_HUB_DB_ID},
                properties={k: v for k, v in profile_props.items() if v}
            )
            profile_page_id = new_profile_page.get('id')
            print(f"✅ [Notion] 已为 '{name}' 创建详细信息档案并自动填充属性。")

        except Exception as e:
            print(f"   !! [Notion] 操作候选人信息库失败: {e}")
            traceback.print_exc()
...
async def process_single_resume_async(file_path, worker_name):
    ...
    prompt = EXTRACT_PROMPT_V2.format(resume_text=resume_text[:8000])
    
    print(f"\n>> [{worker_name}] 正在从 '{filename}' 深度提取信息..."); print("   " + "="*20 + " AI提取中 " + "="*20); 
    
    full_response_content = "";
    # ChatOllama 的 astream 返回的是 AIMessageChunk 对象，需要访问 .content
    ...
    if not extracted_data or not extracted_data.get("name"):
        raise Exception(f"AI未能从 '{filename}' 提取出有效的JSON数据或候选人姓名。")
...
async def batch_mode_high_concurrency():
    ...
    show_desktop_notification(
        "简历处理完成！",
        f"成功: {len(processed_files)}, 跳过: {len(skipped_files)}, 失败: {len(failed_files)}"
    )
...
async def main():
    ...
    print("="*10 + " AI 简历信息提取助理 v2.5 (深度提取与归档版) " + "="*10)
    print(">> 系统启动中...")
    print("-" * 70)
